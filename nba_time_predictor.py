...
if game_id_input:
    ...
    if not lookup_df.empty:
        ...
        st.warning("Game ID not found. Enter stats manually.")
...
with col1:
    ...
    game_clock_elapsed = st.number_input("Game Clock Elapsed", 0, 3600, int(defaults['game_clock_elapsed']))
    pace = st.number_input("Pace", 0.0, 250.0, float(defaults['pace']))
...
with col2:
    ...
    st.write("Game Flags")
    is_close_game = st.checkbox("Close Game", value=bool(defaults['is_close_game']))
...
with col3:
    ...
    foul_rate_20 = st.number_input("Foul Rate (Last 20)", 0.0, 20.0, float(defaults['foul_rate_20']))
    is_foul_cum = st.number_input("Total Fouls", 0, 200, int(defaults['is_foul_cum']))
    is_foul_last4_cum = st.number_input("Fouls Last 4 Min", 0, 100, int(defaults['is_foul_last4_cum']))
    is_timeout_last4_cum = st.number_input("Timeouts Last 4 Min", 0, 20, int(defaults['is_timeout_last4_cum']))
    is_ft_last4_cum = st.number_input("FT Last 4 Min", 0, 100, int(defaults['is_ft_last4_cum']))
    is_turnover_last4_cum = st.number_input("Turnovers Last 4 Min", 0, 50, int(defaults['is_turnover_last4_cum']))
    is_review_cum = st.number_input("Reviews", 0, 20, int(defaults['is_review_cum']))
...
if st.button("Calculate Real-World Time Remaining", type="primary", use_container_width=True):
    ...
    st.metric("Estimated Real-World Time Remaining", f"{minutes}m {seconds:02d}s")
